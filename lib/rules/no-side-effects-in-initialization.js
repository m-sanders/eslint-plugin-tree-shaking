...
const reportSideEffectsInProgram = (context, programNode) => {
  ...
  const DEFINITIONS = {
    ...
    ImportBinding: {
      reportEffectsWhenCalled (definition) {
        ...
        if (
          !getTreeShakingComments(context.getSourceCode().getCommentsBefore(definition.name)).has(
  ...
  const NODES = {
    ...
    ExportDefaultDeclaration: {
      reportEffects (node, scope, options) {
        if (
          getTreeShakingComments(context.getSourceCode().getCommentsBefore(node.declaration)).has(
            COMMENT_NO_SIDE_EFFECT_WHEN_CALLED
          )
        ) {
    ...
    ExportSpecifier: {
      reportEffects (node, scope, options) {
        if (
          getTreeShakingComments(context.getSourceCode().getCommentsBefore(node.exported)).has(
            COMMENT_NO_SIDE_EFFECT_WHEN_CALLED
          )
        ) {
    ...
    FunctionDeclaration: {
      reportEffects (node, scope, options) {
        if (
          node.id &&
          getTreeShakingComments(context.getSourceCode().getCommentsBefore(node.id)).has(
            COMMENT_NO_SIDE_EFFECT_WHEN_CALLED
          )
        ) {
    ...
    Identifier: {
      ...
      reportEffectsWhenCalled (node, scope, options) {
        ...
        } else if (!isPureFunction(node, context.options)) {
    ...
    MemberExpression: {
      ...
      reportEffectsWhenCalled (node, scope, options) {
        ...
        if (!isPureFunction(node, context.options)) {
          context.report(node.property, ERROR_CALL_MEMBER)
        }
      }
    },
